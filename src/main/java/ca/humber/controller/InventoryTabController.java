...
public class InventoryTabController {
	...
	private void handleDeleteItem() {
		...
		boolean confirmed = AlertDialog.showConfirmation("Confirm Delete",
		...
		task.setOnFailed(event -> AlertDialog.showError("Delete Error",
				"Failed to delete item: " + task.getException().getMessage()));
		new Thread(task).start();
	}
	...
	@FXML
	private void handleLowStockFilter() {
		try {
			int threshold = Integer.parseInt(lowStockField.getText());
			if (threshold <= 0) {
				refreshInventoryTable();
				return;
			}
			runInventoryTask(() -> InventoryService.getLowStocks(threshold), "Search Error");

		} catch (NumberFormatException e) {
			AlertDialog.showError("Delete Error", "Invalid number entered in Low Stock field.");
		}
	}
}
